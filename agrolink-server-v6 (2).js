...
// 🛠️ DEV MODE: SMTP yoksa e-posta doğrulamasını atla, kodu logla
// .env dosyasına DEV_MODE=true ekle (production'da ASLA kullanma!)
...
async function sendEmail(to, subject, html, text = null) {
    ...
    try {
        ...
        // 🔒 8 saniyelik hard timeout — SMTP DATA aşamasında takılmayı önler
        // (connectionTimeout/greetingTimeout sadece bağlantı aşamasını keser;
        //  sendMail kendisi kilitlenince dakikalarca beklenir → buton donar)
        const EMAIL_SEND_TIMEOUT_MS = 8000;
        const sendPromise = transporter.sendMail(mailOptions);
        const timeoutPromise = new Promise((_, reject) =>
            setTimeout(() => reject(new Error('SMTP zaman aşımı (8s)')), EMAIL_SEND_TIMEOUT_MS)
        );
...
app.post('/api/auth/register-init', upload.single('profilePic'), async (req, res) => {
    try {
        ...
        // 🛠️ DEV MODE: SMTP yoksa kodu logla, e-posta doğrulamasını atla
        if (DEV_MODE) {
            console.log(`\n⚠️  [DEV MODE] E-posta doğrulama kodu: ${verifyCode} (${cleanEmail})\n`);
            sendWelcomeEmail(cleanEmail, name).catch(() => {});
            return res.status(201).json({
                message: `[DEV] Doğrulama kodu: ${verifyCode} (sunucu loguna bakın)`,
                emailVerificationRequired: true,
        ...
        if (!emailResult.success) {
            // E-posta gönderilemedi ama kullanıcı zaten DB'ye yazıldı.
            // Hesabı silip tekrar denemelerini sağla; böylece "e-posta zaten kayıtlı" hatası oluşmaz.
            console.error('❌ Kayıt doğrulama e-postası gönderilemedi:', emailResult.error);
            await dbRun(`DELETE FROM email_verifications WHERE "userId" = $1`, [userId]).catch(() => {});
            await dbRun(`DELETE FROM users WHERE id = $1`, [userId]).catch(() => {});
    ...
    try {
        ...
        if (user.twoFactorEnabled) {
            ...
            if (!DEV_MODE) {
                ...
                console.log(`\n⚠️  [DEV MODE] 2FA kodu: ${twoFACode} (${user.email})\n`);
            }
            ...
            return res.json({
                ...
                email: user.email,
                message: DEV_MODE
                    ? `[DEV] 2FA kodu: ${twoFACode} (sunucu loguna bakın)`
                    : 'Doğrulama kodu e-posta adresinize gönderildi. Lütfen 6 haneli kodu girin.'
            });
        }
...
app.post('/api/auth/register-verify', async (req, res) => {
    try {
        ...
        const user = await dbGet(
            `SELECT id, name, username, email, role, plan, "profilePic", bio FROM users WHERE id = $1`,
            [verification.userId]
        );
